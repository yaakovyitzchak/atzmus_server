...
//		console.log("LOL",data);
	...
	adminCommand(msg, cs) {
		...
		adminCommand(msg, function(err, rez) {
			var so = {};
			so[cbName] = {
				success: rez,
				error: err
...
this.command = (inputOpts) => {
	var opts = inputOpts && inputOpts.constructor == Object ? inputOpts : {}, 
		...
		if(verified) {
			...
			//	console.log("PROJ!?!", projection, verified);
			}
									...
									if(!s.find(x => x.name == "roles")) {
										...
										self.command({
											...
											query: [
												...
												{
													name:"yeled",
													permissions: "cobs.*:RW, local.*:R, userInfo.users:R[username]"
												},
												{
													name:"eesh",
													permissions: "cobs.*:R"
...
function adminCommand(msg, cb) {
	...
	var opts = t(msg, Object) ? msg : {},
		dbName =    t(opts["database"], String) ? opts.database : "tmp",
	...
	if(t(rs, String)) {
		...
		var dbs = fields.map(function(x) {
			...
			if(x.indexOf(":") < x.length - 1) {
				...
				if(
					o.permission.indexOf("]") >
					o.permission.indexOf("[")
				) {
					var bracks = o.permission.substring(
						o.permission.indexOf("[") + 1,
						o.permission.indexOf("]")
					)
					o.bracks = parseProjection(bracks);
					o.permission = o.permission.substring(
						0,
						o.permission.indexOf("[")
					);
							...
							if(user.password === pwrd) {
								...
								if(role) {
									self.command({
										...
										onfinish(e,r) {
											var myroles = r
											if(myroles && myroles[0]) {
												cb(er,"login success", {
													name: myroles[0].name,
													permissions: myroles[0].permissions
												});
											} else {
												cb("something else happened!" + JSON.stringify(e),null);
											}
										}
									})
								} else {
